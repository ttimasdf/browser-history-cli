...
fn prepared_db() -> Result<PathBuf> {
    let path = get_db_path().map_err(|_| {
        anyhow::anyhow!("Could not find Chrome history DB. Set CHROME_HISTORY_DB env var.")
    })?;
    db::prepare_db(&path).map_err(|_| {
        anyhow::anyhow!(
            "Chrome history DB not found: {}. Set CHROME_HISTORY_DB env var.",
            path.display()
        )
    })
}
...
pub fn annotations(
    ...
    let db_path = prepared_db()?;
    let _guard = CleanupGuard(db_path.clone());
    chromium::annotations(&db_path, _from, _to, _limit, _format)
}
...
pub fn contexts(
    ...
    let db_path = prepared_db()?;
    let _guard = CleanupGuard(db_path.clone());
    chromium::contexts(&db_path, _from, _to, _limit, _format)
}
...
pub fn summary(
    ...
    let db_path = prepared_db()?;
    let _guard = CleanupGuard(db_path.clone());
    chromium::summary(&db_path, "Chrome", _from, _to)
}
